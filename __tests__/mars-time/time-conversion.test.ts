...
describe('MarsTimeCalculator', () => {
  ...
  describe('getMTC', () => {
    ...
    test('handles midnight boundary correctly', () => {
      ...
      const hours = parseInt(mtc.split(':')[0] ?? '0');
  ...
  describe('getLTST', () => {
    test('returns valid time format for Curiosity location', () => {
      const testDate = new Date('2023-07-01T12:00:00.000Z');
      const ltst = MarsTimeCalculator.getLTST(
        testDate,
        ROVER_LOCATIONS.curiosity?.longitude ?? 137.4417
      );
    ...
    test('returns valid time format for Perseverance location', () => {
      ...
      const ltst = MarsTimeCalculator.getLTST(
        testDate,
        ROVER_LOCATIONS.perseverance?.longitude ?? 77.4509
      );
    ...
    test('handles negative longitudes correctly', () => {
      ...
      const hours = parseInt(ltst.split(':')[0] ?? '0');
      expect(hours).toBeGreaterThanOrEqual(0);
      expect(hours).toBeLessThan(24);
    });
  });
  ...
  describe('getMissionSol', () => {
    ...
    test('calculates positive sol numbers for future dates', () => {
      ...
      const futureDate = new Date(
        landingDate.getTime() + 10 * 24 * 60 * 60 * 1000
      ); // 10 Earth days later
  ...
  describe('angle conversions', () => {
    test('converts degrees to radians correctly', () => {
      ...
      expect(MarsTimeCalculator.degreesToRadians(90)).toBeCloseTo(
        Math.PI / 2,
        10
      );
      expect(MarsTimeCalculator.degreesToRadians(180)).toBeCloseTo(Math.PI, 10);
      expect(MarsTimeCalculator.degreesToRadians(360)).toBeCloseTo(
        2 * Math.PI,
        10
      );
    });
    ...
    test('converts radians to degrees correctly', () => {
      ...
      expect(MarsTimeCalculator.radiansToDegrees(Math.PI / 2)).toBeCloseTo(
        90,
        10
      );
      expect(MarsTimeCalculator.radiansToDegrees(Math.PI)).toBeCloseTo(180, 10);
      expect(MarsTimeCalculator.radiansToDegrees(2 * Math.PI)).toBeCloseTo(
        360,
        10
      );
    });
